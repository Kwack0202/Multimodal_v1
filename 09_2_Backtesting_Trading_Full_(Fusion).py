        ...
        for day in days:
            ...
            for model in models:
                ...
                for voting in votings:
                    df = pd.read_csv(f"./Backtesting/trading_signal/full_period/Fusion/{ticker}_{voting}_{day}_{model}_signal.csv", index_col=0)
                    TA = pd.read_csv(f"./Backtesting/trading_signal/full_period/LSTM/{ticker}_{day}_signal.csv", index_col=0).iloc[:, 0:5]
                    ...
                    if not os.path.exists(f'./Backtesting/trading_results/full_period/Fusion/'):
                        os.makedirs(f'./Backtesting/trading_results/full_period/Fusion/')
                        
                    merged_df.to_csv(f'./Backtesting/trading_results/full_period/Fusion/{ticker}_{voting}_{day}_{model}_signal.csv', index=True)