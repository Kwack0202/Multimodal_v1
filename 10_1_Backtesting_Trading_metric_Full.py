...
for ticker in tickers:
    for day in days:
        for model in ['LSTM', 'VIT']:
            for time_step in time_steps:
                ...
                # 데이터 프레임 Load
                df = pd.read_csv(f'./Backtesting/trading_results/full_period/{model}/{ticker}_{day}_{time_step}_signal.csv', index_col=0)
                ...
                if not os.path.exists(f'./Backtesting/Final_results/full_period/'):
                        os.makedirs(f'./Backtesting/Final_results/full_period/')
                        
                df.to_csv(f"./Backtesting/Final_results/full_period/{ticker}_{model}_{day}_{time_step}.csv")
        ...
        for model in ['Fusion']:
            ...
            for voting in votings:
                ...
                for ml in ['lgb', 'SVM', 'xgb']:
                    ...
                    # 데이터 프레임 Load
                    df = pd.read_csv(f'./Backtesting/trading_results/full_period/{model}/{ticker}_{voting}_{day}_{ml}_signal.csv', index_col=0)
                    df.columns.values[5:7] = ["Predicted_results", "action"]
                    ...
                    # 초기 투자금액은 처음 Buy 시점의 Close 값으로 설정
                    initial_investment = df[df['action'] == 'Buy']['Close'].iloc[0]
                    ...
                    df.to_csv(f"./Backtesting/Final_results/full_period/{ticker}_{model}_{voting}_{day}_{ml}.csv")
    ...
    summary_df.to_csv("Final_results_Summary_full_period.csv")